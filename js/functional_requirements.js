$(document).ready(function(){
  ...
  // functional_requirements auto_increment ID
  const id_result = JSON.parse(localStorage.getItem("db_game_design_best_practices"));
  const auto_increment = id_result.tables.functional_requirements.auto_increment;
  $('[name="identification"]').val(auto_increment);

  $('#createFunctionalRequirementsForm').on('reset', function(){
    document.location.reload();
  });

  $('#createFunctionalRequirementsForm').submit(function() {
    // Get all the forms elements and their values in one step
    var values = $(this).serialize();

    var functional_requirement_values = {};
    values = values.split("&");

    for(i=0; i<values.length; i++){
      const value = values[i].split("=");
      Object.defineProperty(functional_requirement_values, value[0], {value: decodeURIComponent(value[1])});
    }
    
    console.log(functional_requirement_values);
    db.insert("functional_requirements", functional_requirement_values);
    db.commit();
  });

  var state = {
    querySet: db.queryAll("functional_requirements"),
    page: 1,
    rows: 3,
    window: 5
  }
  ...
  function pageButtons(pages) {
    ...
    $(".fa-pencil-square-o").click(function(){
      ...
      $("#edit").show();
      $("#create").hide();
      const functional_requirement = db.queryAll("functional_requirements", { query: {ID: last_id_num} })[0];
      console.log(functional_requirement)
      $('[name="edit_identification"]').val(functional_requirement.ID);
      $('[name="edit_description"]').val(functional_requirement.description);
    });

    $("#functionalRequirementsEditButton").click(function() {
    ...
    $("#edit_functional_requirement").click(function(){
      // Get all the forms elements and their values in one step
      var values = $("#editFunctionalRequirementsForm").serialize();
      ...
      console.log("EDITING FUNCTIONAL REQUIREMENT WITH ID " + last_id_num);
      db.update("functional_requirements", {ID: last_id_num}, function(row){
        row.description = functional_requirement_values.edit_description;
    ...
    $("#delete_functional_requirement").click(function(){
      console.log("DELETING FUNCTIONAL REQUIREMENT WITH ID " + last_id_num);
      db.deleteRows("functional_requirements", {ID: last_id_num});
      db.commit();
      document.location.reload();
    });
  }

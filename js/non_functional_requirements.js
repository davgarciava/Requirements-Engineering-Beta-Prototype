$(document).ready(function(){
  ...
  // non-functional_requirements auto_increment ID
  const id_result = JSON.parse(localStorage.getItem("db_game_design_best_practices"));
  const auto_increment = id_result.tables.non_functional_requirements.auto_increment;
  $('[name="identification"]').val(auto_increment);

  $('#createNonFunctionalRequirementsForm').on('reset', function(){
    document.location.reload();
  });

  $('#createNonFunctionalRequirementsForm').submit(function() {
    // Get all the forms elements and their values in one step
    var values = $(this).serialize();

    var non_functional_requirement_values = {};
    values = values.split("&");

    for(i=0; i<values.length; i++){
      const value = values[i].split("=");
      Object.defineProperty(non_functional_requirement_values, value[0], {value: decodeURIComponent(value[1])});
    }
    
    console.log(non_functional_requirement_values);
    db.insert("non_functional_requirements", non_functional_requirement_values);
    db.commit();
  });

  var state = {
    querySet: db.queryAll("non_functional_requirements"),
    page: 1,
    rows: 3,
    window: 5
  }
  ...
  function pageButtons(pages) {
    ...
    $(".fa-pencil-square-o").click(function(){
      ...
      $("#edit").show();
      $("#create").hide();
      const non_functional_requirement = db.queryAll("non_functional_requirements", { query: {ID: last_id_num} })[0];
      console.log(non_functional_requirement)
      $('[name="edit_identification"]').val(non_functional_requirement.ID);
      $('[name="edit_description"]').val(non_functional_requirement.description);
    });

    $("#nonFunctionalRequirementsEditButton").click(function() {
    ...
    $("#edit_non_functional_requirement").click(function(){
      // Get all the forms elements and their values in one step
      var values = $("#editNonFunctionalRequirementsForm").serialize();
      ...
      console.log("EDITING NON-FUNCTIONAL REQUIREMENT WITH ID " + last_id_num);
      db.update("non_functional_requirements", {ID: last_id_num}, function(row){
        row.description = non_functional_requirement_values.edit_description;
    ...
    $("#delete_non_functional_requirement").click(function(){
      console.log("DELETING NON-FUNCTIONAL REQUIREMENT WITH ID " + last_id_num);
      db.deleteRows("non_functional_requirements", {ID: last_id_num});
      db.commit();
      document.location.reload();
    });
  }
